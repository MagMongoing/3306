...
NUM=$7
COND=`date --date="$NUM days ago" +%F`
COND2=`date --date="$NUM days ago" +%Y%m%d`

WHERE_SQL="create_time<'"$COND"'"
...
mysql -uarchive -p$D_PASS -h$D_HOST $D_DB -e "alter table $table rename to $D_TABLE;alter table $D_TABLE row_format=compressed"
#LOG FILE
LOG=./log/$S_TABLE.sql
echo " "     >>$LOG
...
echo " "     >>$LOG
echo $WHERE_SQL
/usr/bin/pt-archiver --source $SOURCE --dest $DEST  --where "$WHERE_SQL" $SLAVE  $CHARSET --bulk-insert  --commit-each --limit=$LIMIT --progress=$PROGRESS   --statistics --why-quit $DRY>>$LOG
